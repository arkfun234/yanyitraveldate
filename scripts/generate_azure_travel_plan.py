...
AB_FIELD_GROUPS = {
    ...
    "user_conditions": ("user_conditions", "userconditions"),
...
def build_prompt(
    ...
    user_conditions = ab_result.get("user_conditions")
    if not isinstance(user_conditions, dict):
        user_conditions = ab_result.get("ai_travel_conditions")
    if not isinstance(user_conditions, dict) and isinstance(ab_result.get("research_context"), dict):
        user_conditions = ab_result["research_context"].get("user_conditions")
    user_conditions = user_conditions if isinstance(user_conditions, dict) else {}
    user_conditions_text = json_text(user_conditions, MAX_AB_SECTION_CHARS)
...
この入力は A/B 比較ではなく、すでに統合推薦として再ランキングされた結果です。日本語Markdownで、対象ユーザー専用の福井県内旅行プランを作成してください。
...
今回の旅行条件は、心理傾向と同時に考慮してください。travel_request は希望・行きたい場所・避けたいこと、transport は移動順序と移動負担、trip_duration は時間配分と滞在時間、spending_style は直接的な予算額ではなく旅行中のお金の使い方の傾向として扱ってください。
心理傾向と今回の旅行条件の両方を踏まえて、時間配分、移動順序、滞在時間、推薦理由を調整してください。

## 推薦情報の使い方
- integrated_recommendations を最優先で使ってください。
- 各候補の recommendation_reason と b_need_match は内部根拠として必ず読み、旅行者向けの自然な推薦理由に言い換えてください。
- matched_a_cluster は旅行者タイプの基盤、matched_b_cluster は自由記述に基づく補助的な旅行ニーズとして扱ってください。
- matched_a_cluster.related_places や matched_b_cluster.top_keywords / top_areas は、訪問順序、テーマ説明、補助候補の判断材料として参照してよいですが、integrated_recommendations より優先しないでください。
- 旅行プランは原則として integrated_recommendations に含まれるスポットを中心に作成してください。
...
- recommendation_reason、matched_keywords、score_debug、debug score、purpose_tags、trait_tags、spot_type、description、b_need_match などのデータ項目名を、旅行計画の本文にそのまま出力しないでください。
- debug score や内部スコアは本文に出さず、必要な場合だけ内部参考として解釈してください。
- integrated_recommendations に含まれない観光スポットを、主な訪問先として勝手に追加しないでください。
- integrated_recommendations に含まれないスポットを追加する必要がある場合は、必ず「補助候補」として扱い、なぜ追加したのかを明記してください。
- 推薦候補、matched_a_cluster、matched_b_cluster と関係のないスポットを自由に追加しないでください。
- 道の駅、観光案内所、ターミナル、駅などが含まれる場合は、休憩・交通・補助点としてのみ扱い、主要目的地にしないでください。
- 交通、飲食、休憩地点を追加する必要がある場合は、必ず「補助点」と明記してください。
- 候補から3〜5か所を選び、trip_duration に合わせた現実的な旅行ルートにしてください。
- 移動距離と時間配分が現実的になるようにし、位置関係や営業状況を断定できない場合は「要確認」としてください。
- ユーザーに見える旅行計画は、観光客向けの案内文として書いてください。研究用のフィールド説明、JSON風の項目名、スコア説明、デバッグ説明は本文に出さないでください。
- 研究用の根拠を示す場合は、最後の「抽出された統合推薦情報」に短い自然文で要約してください。生データやJSONを貼り付けないでください。
- ただし、Markdown構造は必ず維持してください。見出し記号 ##、訪問スポット用の ###、箇条書き -、太字 ** は必ず使ってください。
- 各訪問スポット、出発、移動、昼食、休憩を行程として書く場合は、必ず「### 09:30 - 10:00 福井駅出発」のように ### で始めてください。時間だけの通常行にしないでください。
- 「旅行の流れ」は必ず「## 旅行の流れ」という見出しで書いてください。

## 参照用入力データ（出力には貼り付けない）
以下は旅行プランを作成するための内部参照データです。最終回答にこの見出しやJSON、生データをそのまま出力しないでください。
...
## 出力形式
以下のMarkdown構造を厳守してください。自然な文章で埋めてください。ただし、recommendation_reason、matched_keywords、score_debug などの内部字段名は絶対に本文に出さないでください。

# 福井県 旅行プラン

## 利用した統合推薦情報
- **旅行者タイプの基盤**：自然な説明文
- **補助的に参照した旅行ニーズ**：自然な説明文
- **統合推薦で重視した点**：自然な説明文

## 今回の旅行条件の反映
- **重視した希望・避けたいこと**：自然な説明文
- **交通手段に合わせた調整**：自然な説明文
- **旅行時間に合わせた時間配分**：自然な説明文
- **お金の使い方に合わせた調整**：自然な説明文
...
### 10:30 - 12:00 推薦スポット名（エリア名）
- **内容**：自然な説明文
- **滞在時間**：約1時間30分
- **理由**：この旅行者に合う理由を自然に説明
- **補足**：必要があれば自然に説明

### 12:30 - 14:00 推薦スポット名（エリア名）
- **内容**：自然な説明文
- **滞在時間**：約1時間30分
- **理由**：自然な説明文

### 14:30 - 16:00 推薦スポット名（エリア名）
- **内容**：自然な説明文
- **滞在時間**：約1時間30分
- **理由**：自然な説明文
...
## 抽出された統合推薦情報
- **推薦スポット**：自然な摘要。JSONや内部字段名は出さない。
- **旅行者タイプと旅行ニーズ**：自然な摘要。JSONや内部字段名は出さない。
- **行程に反映した点**：自然な摘要。JSONや内部字段名は出さない。

重要な制約：
1. recommendation_reason、matched_keywords、score_debug などの内部字段名は絶対に本文に出さない。
2. ただし、見出し記号 ##、###、箇条書き -、太字 ** は必ず使う。
3. 各訪問スポットは必ず ### で始める。
4. フロントエンドのMarkdownレンダリングに合わせるため、必ず「## 旅行の流れ」と書く。
5. 「09:30 - 福井駅出発」のような時間行も、必ず「### 09:30 - 福井駅出発」にする。
"""
